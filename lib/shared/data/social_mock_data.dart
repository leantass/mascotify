  ...
  if (pets.isNotEmpty) {
    threads.add(
      MessageThread(
        ...
        pet: pets[0],
        relatedLabel: 'Chat activo sobre ${pets[0].name}',
        lastMessage:
        ...
        messages: const [
          MessageEntry(
            text:
                'Hola, esta mascota nos dio muy buena impresión y queríamos conversar un poco más.',
            timestamp: '09:06',
            isMine: false,
          ),
          MessageEntry(
...
List<SocialInboxEntry> buildMockSocialInboxEntries([List<Pet>? sourcePets]) {
  final pets = _resolveSocialPets(sourcePets);
  final entries = <SocialInboxEntry>[];
...
List<SavedProfileEntry> buildMockSavedProfiles([List<Pet>? sourcePets]) {
  final pets = _resolveSocialPets(sourcePets);
  final entries = <SavedProfileEntry>[];
